...
//		c.wick_contract();
	...
	//	cout << "effectively reduces the container size by the number of elementswick contraction for c_ij = " << i << " " << j << endl;
		...
		{
			main_logger->info("Loading contractions for c_{}.{}",i,j);
			if(j<=i)
...
void Manager::load_numerical_results_and_compute_tavg_corr()
{
	auto main_logger = spdlog::get("main");
	...
	main_logger->info("All diagram files found, loading them");
	main_logger->flush();

	//map<string, map<string,complex<double>>> computed;

	map<string, vector<vector<complex<double>>>> computed;
	for(const auto d_file : files.diagram_files)
	{
			...
			for(const auto &d : new_diags)
			{
				...
				{
					vector<vector<complex<double>>> zeros(lat.nt, vector<complex<double>>(lat.nt, 0));
					computed[d.first]=zeros;
					for(size_t dt=0; dt<lat.nt; ++dt)
					for(size_t t=0; t<lat.nt; ++t)
					{
						if(d.second[dt][t]!=complex<double>(0))
							computed[d.first][dt][t]=d.second[dt][t];
					}
				}
				else
				{
			...
			main_logger->info("Parsed file " + d_file);
			main_logger->flush();
	}

	main_logger->info("All diagrams loaded");
	main_logger->flush();
	...
	for(size_t j=0; j<ops.size(); ++j)
	{
		if(j<=i)
		{
			int idx = i*ops.size() + j;
		//if(i>285)
		//{
			try
			{
				main_logger->debug("loading numerical results for correlator " + to_string(idx));
				main_logger->flush();
	...
	main_logger->info("Done loading numerical results");
	main_logger->flush();

	//auto tmp = corrs[286].diags[0].traces[0];
	//cout << tmp.name() << " has (end of load numerical) size " << tmp.numerical_value.size() << endl << flush;
	//tmp = corrs[286].diags[0].traces[1];
	//cout << tmp.name() << " has size " << tmp.numerical_value.size() << endl << flush;
  //tmp = corrs[286].diags[1].traces[0];
	//cout << tmp.name() << " has size " << tmp.numerical_value.size() << endl << flush;
}
...
void Manager::compute_time_average_correlators()
{
	...
	//	if(i>285)
	//	{
			//auto tmp = corrs[i].diags[0].traces[0];
		//	cout << tmp.name() << " has size " << tmp.numerical_value.size() << endl << flush;

		main_logger->debug("Computing t average for correlator " + to_string(i));
		main_logger->flush();
		corrs[i].compute_time_average_correlators();
		//}
	}
	main_logger->info("Computed t average of all correlators");
}
